...
const [pendingAlerts, setPendingAlerts] = useState<PendingBlockAlert[]>([]);
  ...
  async function init() {
    ...
    if (uid) {
      try {
        ...
        await loadAll(uid);   // ⬅️ carga inicial desde la nube
      } finally {
    ...
    async (_event: AuthChangeEvent, session: Session | null) => {
      ...
      if (uid) {
        try {
          setLoadingCloud(true);
          await seedIfEmpty(uid);
          await loadAll(uid);   // ⬅️ recarga cuando haces login/logout
        } finally {
      ...
      // ✅ Hemos rehidratado tras cambio de sesión
      hydratedRef.current = true;
    }
  );
...
function editBlockTotalFromSlice(slice: TaskSlice) {
  ...
  setSlices(prev => {
    ...
    const plan = planificarBloqueAuto(
      ...
      nuevoTotal,
      w,
      fromLocalISO(startF), // usa el helper fromLocalISO que ya tienes arriba
      base,
      restantes,
      overrides
...
// 2) Bloques / Slices
    ...
    if (oData) {
      const obj: Record<
        string,
        Record<string, { extra?: number; sabado?: boolean; domingo?: boolean; vacacion?: boolean }>
      > = {};
      ...
      setOverrides(obj); // ⬅️ MUY IMPORTANTE: sustituye todo el estado
    }
      ...
      {pendingAlerts.length > 0 && (
        <div style={alertBox} className="no-print">
          ...
          <ul style={alertList}>
            {pendingAlerts.map((b) => (
              <li
                key={`alert-${b.trabajadorId}-${b.taskId}-${b.fecha}`}
              >
                <strong>{b.fecha}</strong>
                {" · "}
                {b.trabajadorNombre}
                {" · "}
                <span style={{ fontWeight: 700 }}>{b.producto}</span>
                {b.descripcion && (
                  <>
                    {" · "}
    ...
    : `Quedan ${b.remainingDays} día${
        b.remainingDays === 1 ? "" : "s"
          ...
          <div style={{ fontSize: 11, marginTop: 4 }}>
            Valida estos bloques (✔) para que dejen de aparecer en la lista.
          </div>
        </div>
      )}
...
return (
  <div
    style={baseStyle}
    onMouseEnter={(e) => {
      (e.currentTarget as HTMLDivElement).style.boxShadow = dayCellHover.boxShadow!;
      (e.currentTarget as HTMLDivElement).style.transform = dayCellHover.transform!;
    }}
    onMouseLeave={(e) => {
      (e.currentTarget as HTMLDivElement).style.boxShadow = baseStyle.boxShadow || "none";
